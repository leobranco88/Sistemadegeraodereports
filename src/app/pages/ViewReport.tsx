...
export function ViewReport() {
  ...
  // For demo, use the first report or find by student ID
  const report = mockReports.find(r => r.studentId === reportId) || mockReports[0];
  ...
  const radarData = report.competencies.map(comp => ({
    subject: comp.name.split(' ')[0], // First word of competency name
    value: comp.rating,
    fullMark: 5,
  }));

  const averageRating = report.competencies.reduce((sum, comp) => sum + comp.rating, 0) / report.competencies.length;
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 mb-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-[#070738] text-white">
                  {report.class}
                </span>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-[#9CA3AF]">Professor: </span>
              <span className="text-[#3D3D3D]">{report.professorName}</span>
            </div>
            <div>
              <span className="text-[#9CA3AF]">Coordenador: </span>
              <span className="text-[#3D3D3D]">{report.coordinatorName}</span>
            </div>
            <div>
              <span className="text-[#9CA3AF]">Ciclo: </span>
              <span className="text-[#3D3D3D]">{report.evaluation}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-8 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="text-center">
              ...
              <p className="text-4xl text-[#070738] mb-1">{report.attendance}%</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-[#9CA3AF] mb-2">Nota do Teste</p>
              <p className="text-4xl text-[#070738] mb-1">{report.testScore}%</p>
            </div>
            <div className="text-center">
            ...
            <div className="flex flex-col items-center justify-center">
              <div className="text-center">
                ...
                <div className="w-full bg-gray-200 rounded-full h-3 mb-2">
                  <div
                    className={`h-3 rounded-full ${getRatingColor(averageRating)}`}
            ...
            {report.competencies.map((comp) => (
              <div key={comp.id} className="border border-gray-200 rounded-lg p-6">
                ...
                <div className="w-full bg-gray-200 rounded-full h-2 mb-6">
                  <div
                    className={`h-2 rounded-full ${getRatingColor(comp.rating)}`}
                    style={{ width: `${(comp.rating / 5) * 100}%` }}
                  />
                </div>
                <div className="grid md:grid-cols-3 gap-6">
                  <div>
                    <h4 className="text-sm text-[#EC5800] mb-2">O que vejo</h4>
                    <p className="text-sm text-[#3D3D3D] leading-relaxed">{comp.whatISee}</p>
                  </div>
                  <div>
                    <h4 className="text-sm text-[#EC5800] mb-2">Por que importa</h4>
                    <p className="text-sm text-[#3D3D3D] leading-relaxed">{comp.whyItMatters}</p>
                  </div>
                  <div>
                    <h4 className="text-sm text-[#EC5800] mb-2">O que fazer</h4>
                    <p className="text-sm text-[#3D3D3D] leading-relaxed">{comp.whatToDo}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            <div className="text-center">
              <div className="border-t-2 border-gray-300 pt-2 mb-2">
                <p className="text-sm text-[#3D3D3D]">{report.coordinatorName}</p>
              </div>
